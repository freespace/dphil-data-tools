...
class CSVReader(object):
          ...
          for c in line:
            if c.isalpha():
              ...
              for schar in splitchars:
                if schar in line:
                  return map(str.strip,line.split(schar))
          else:
            done = True
    return None
  ...
  def mat(self):
    ...
    def _load_liberal():
      delim = ' '
      with open(self._csvfile) as f:
        contents = f.read()
        commacnt = len(filter(lambda c:c==',', contents))
        tabcnt = len(filter(lambda c:c=='\t', contents))

        if commacnt > 0 and commacnt > tabcnt:
          delim = ','

        if tabcnt > 0 and tabcnt > commacnt:
          delim = '\t'

      return numpy.genfromtxt(self._csvfile, skip_header=1, invalid_raise=False, delimiter=delim)

    def _load_lecroy_py():
      # files generated by lecroy.py, which are csv exports of Lecroy binary
      # waveforms
      self._csv_source = 'lecroy.py'
      with open(self._csvfile) as fh:
        lines = fh.readlines()
        assert '# {' in lines[0]
        return numpy.loadtxt(lines, delimiter=',')
    ...
    if self._mat is None:
      ...
      for loader in loaders:
        try:
          ...
          mat = loader()
          import sys
          sys.stderr.write('CSV loading using loader %s\n'%(loader))
          break
        except Exception, ex:
