...
Then in each section it determines when the fluorescence value rises above
95% of the maximum and when it falls below 95% of the maximum, and takes
the arithmatic mean of the two to be the "center" of fluorescence distribution.
This is done for each row and the final value for the section if the mean over
all rows.
...
Rationale for tracking the the center of fluorescence this way is that tracking
just the maximum or when 95% of maximum is exceeded results is a lot of noise
due to flucuations in the instrument reading.
"""
...
def compute_growth(npz, debug):
  ...
  for secidx, rowspan in zip(xrange(nsections), sec_ratio):
    ...
    rdx_sum = 0
    # not every row will exceed the threshold, so count the number of times
    # it does happen
    exceed_cnt = 0

    threshold = 0.8 * section.max()
    for row in section:
      ...
      for rdx, val in enumerate(reversed(row)):
        if val >= threshold:
          rdx_sum += (row.size - rdx - 1)
          # don't double inc the exceed_cnt here. If a pixel exceeds the
          # threshold when scanning forward, we will see it again when
          # scanning backwards

    mean_rdx  = rdx_sum / 2 / exceed_cnt

    mean_z = scandata.zpositionvec[mean_rdx]
    mean_z_vec.append(mean_z)

    mean_rdx_vec.append(mean_rdx)
  ...
  if debug:
    ...
    for secidx in xrange(nsections):
      ...
      mat[startrow:endrow,mean_rdx_vec[secidx]] = np.ones(endrow-startrow) * mat.max()
...
def main(scan_id=None, debug=False):
  ...
  outputfile = scan_id + '_growth_mean.npz'
